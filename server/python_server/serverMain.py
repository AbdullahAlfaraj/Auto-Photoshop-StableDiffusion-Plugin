...
async def txt2ImgRequest(payload):
    ...
    async with httpx.AsyncClient() as client:
        response = await client.post(
            url=f"{sd_url}/sdapi/v1/txt2img", json=payload, timeout=None
        )
        r = response.json()
        ...
        dir_fullpath, dirName = serverHelper.getUniqueDocumentDirPathName(
            uniqueDocumentId
        )
...
async def getInitImageHandle(request: Request):
    ...
    for i in range(numOfAttempts):
        try:
            ...
            print(
                f"exception:fail to read an image file {image_path}, will try again {i} of {numOfAttempts}"
            )
...
async def maskExpansionHandler(request: Request):
    ...
    try:
        ...
        )  # save a copy of the mask for debugging
        ...
        )  # save a copy of the mask of the expanded_mask for debugging
...
async def loadHistory(request: Request):
    ...
    try:
        ...
        settings_paths = glob.glob(
            f"./output/{uniqueDocumentId}/*.json"
        )  # note: why is we are not using settings_paths?
        ...
        for image_path in image_paths:
            ...
            metadata_dict = metadata_to_json.createMetadataJsonFileIfNotExist(
                image_path
            )
...
async def list_available_loras():
    ...
    try:
        ...
        candidates = (
            ...
            + glob.glob(
                os.path.join(shared.cmd_opts.lora_dir, "**/*.safetensors"),
                recursive=True,
            )
            + glob.glob(
                os.path.join(shared.cmd_opts.lora_dir, "**/*.ckpt"), recursive=True
            )
        )
...
@app.post("/controlnet/filter")
async def filter(
    keyword: str = Body("All", title="keyword to filter by"),
    ...
    try:
        [
            ...
            default_option,
            default_model,
        ] = global_state.filter_selected_helper(keyword, preprocessor_list, model_list)
    except Exception as e:
