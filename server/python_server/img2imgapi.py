...
async def img2ImgRequest(sd_url, payload):
    ...
    if len(init_img_mask_name) > 0:
        ...
        payload[
            "mask"
        ] = init_img_mask_str  # there is only one mask, unlike 'init_images' which is of type array
    ...
    async with httpx.AsyncClient() as client:
        response = await client.post(
            url=f"{sd_url}/sdapi/v1/img2img", json=payload, timeout=None
        )
        ...
        for i in r["images"]:
            ...
            response2 = await client.post(
                url=f"{sd_url}/sdapi/v1/png-info", json=png_payload, timeout=None
            )
