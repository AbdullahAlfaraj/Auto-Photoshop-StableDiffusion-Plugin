  ...
  async function refreshModelsHorde () {
      try{
          ...
          g_models_horde.sort(function(a, b){return b.count - a.count});
    ...
    for (let model of g_models_horde) {
      ...
      const model_item_element = addHordeModelMenuItem(model_html_tile,model.name)
      
      document.getElementById('mModelsMenuHorde').appendChild(model_item_element)
    }
  ...
  function getModelHorde(){
     return  [...document.getElementsByClassName('mModelMenuItemHorde')].filter(e => e.selected == true)[0].dataset.name
  }
  ...
  document.getElementById('btnRefreshModelsHorde').addEventListener('click',async ()=>{
